...
const notifierEmail = (email, nom, montant, objectif) => {
  const sujet = encodeURIComponent('Paiement valide — AWF Members')
  const corps = encodeURIComponent(
    'Bonjour ' + nom + ',\n\nVotre paiement de ' + (montant || 0).toLocaleString() + ' FCFA pour "' + (objectif || '') + '" a ete valide avec succes.\n\nMerci pour votre contribution.\n\n— AWF Members\nPowered by Olivier Martial KONO'
  )
...
const LoginForm = ({ onAuth }) => {
  ...
  const handleLogin = async () => {
    ...
    const { data: membre } = await supabase.from('membres').select('*').eq('email', email).maybeSingle()
...
{"montant": nombre|null, "devise": "FCFA", "reference": string|null, "expediteur": string|null, "recepteur": string|null, "numero_recepteur": string|null, "date": string|null, "mode": "Orange Money"|"MTN MoMo"|"Cash"|"Autre", "destinataire_valide": true|false, "confiance": "haute"|"moyenne"|"faible"}` }
  ...
  const submit = async () => {
    ...
    let { data: obj } = await supabase.from('objectifs').select('id').eq('nom', form.objectif_nom).maybeSingle()
    if (!obj) {
